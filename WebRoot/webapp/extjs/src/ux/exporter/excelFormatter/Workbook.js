...
Ext.define("Ext.ux.exporter.excelFormatter.Workbook", {
  ...
  addStyle: function(config) {
    ...
    var yourStyleId=style.getId();
//    alert("yourStyleId="+yourStyleId);
    
    var exists=false;
    
    Ext.Array.each(this.styles, function(style, index) {  
//    	alert(""+style.getId()+"==?"+yourStyleId);
    	if(style.getId()==yourStyleId){
    		exists=true;
    		return;//此处需要用return而不是break
    	}
	 });
    //重复的要去掉,否则生成的excel打不开
    if(!exists)
