...
public class Word {
		...
		public Word(boolean visible) throws Exception {
			if (word == null) {
...
//	        Dispatch.call(selection, "MoveRight", new Variant(1), new Variant(1));  
		...
		public void insertTitle(String newText) {
			setFont(true, false, false, "0,0,0,22", "22", "宋体");
			Dispatch.put(selection, "Text", newText);
	        // Get the current selection within Word at the moment.  
	        // a new document has just been created then this will be at  
	        // the top of the new doc 获得选 中的内容，如果是一个新创建的文件，因里面无内容，则光标应处于文件开头处  
...
//	        Dispatch.call(selection, "MoveRight", new Variant(1), new Variant(1));  
		...
		public void insertFormatStr(String text,String ziti,int size,String color,Boolean hei,Boolean xie,int i) {  
	       ...
	       //         new Variant(paragraphCount)).toDispatch(); // 最后一段（也就是刚插入的）  
	        ...
	        //        .toDispatch();
	    	Dispatch selection1 = Dispatch.get(word, "Selection").toDispatch();
	    	Dispatch.call(selection1, "EndKey", "6");
	    	Dispatch.put(selection1,"Text",text);
	    	
	    	
	        Dispatch font = Dispatch.get(selection1, "Font").toDispatch();  
	        Dispatch.put(font, "Bold", new Variant(hei)); // 设置为黑体  
	        Dispatch.put(font, "Italic", new Variant(xie)); // 设置为斜体  
	        Dispatch.put(font, "Name", new Variant(ziti)); //  
	       ...
	       // Dispatch.call(selection, "TypeParagraph");// 插入一个空行  
	        Dispatch alignment = Dispatch.get(selection, "ParagraphFormat")  
	                .toDispatch();// 段落格式  
	        Dispatch.put(alignment, "Alignment", i); // (1:置中 2:靠右 3:靠左) 
	        //moveRight(1);
	    }
		...
		public void createTable(String pos, int numCols, int numRows) {
			if (find(pos)) {
				...
				Dispatch newTable = Dispatch.call(tables, "Add", range,
						new Variant(numRows), new Variant(numCols),new Variant(1)).toDispatch();
				Dispatch.call(selection, "MoveRight");
				//   Dispatch.call(selection, "MoveRight", "3", 1);
			}
		}
